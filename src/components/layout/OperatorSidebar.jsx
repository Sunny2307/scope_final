...
import { FiHome, FiMenu, FiUpload, FiUsers, FiUserCheck, FiUserPlus, FiFileText } from 'react-icons/fi';
import { Link } from 'react-router-dom';

const OperatorSidebar = ({ isSidebarOpen, onSidebarToggle, activeItem }) => {
    const navItems = [
        { name: 'Home', icon: FiHome, path: '/operator/dashboard' },
        { name: 'Import Excel', icon: FiUpload, path: '/operator/import-excel' },
        { name: 'Pending Students', icon: FiUsers, path: '/operator/pending-students' },
        { name: 'Student Management', icon: FiUserCheck, path: '/operator/student-management' },
        { name: 'Guide Management', icon: FiUserPlus, path: '/operator/guide-management' },
        { name: 'Report Generation', icon: FiFileText, path: '/operator/report-generation' },
    ];
    ...
    return (
        <aside className={`bg-white transition-all transition-width duration-500 ease-in-out h-full border-r border-gray-200 z-30 flex flex-col ${isSidebarOpen ? 'w-64' : 'w-16'} overflow-x-hidden`} style={{ minWidth: isSidebarOpen ? '16rem' : '4rem' }}>
            <button
                className={`group flex items-center w-full hover:bg-cyan-100 ${isSidebarOpen ? 'py-3 px-5 justify-start' : 'p-3 justify-center'}`}
