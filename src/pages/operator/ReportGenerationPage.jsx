...
const ReportGenerationPage = () => {
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      const response = await api.get('/api/auth/operator/monthly-report', {
        params: {
          year: selectedYear,
          month: selectedMonth,
        },
  ...
  const downloadPDF = async () => {
    ...
    doc.setFontSize(14);
    doc.setFont('helvetica', 'bold');
    doc.text('Monthly Scholarship Report', pageWidth / 2, 22, { align: 'center' });
    ...
    // Save PDF
    const fileName = `Monthly_Report_${monthNames[selectedMonth - 1]}_${selectedYear}.pdf`;
    doc.save(fileName);
  };
